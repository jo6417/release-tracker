# -*- coding: utf-8 -*-
"""브리핑 — "지금 상태" 칸들 (오늘 공개·공개 임박·완결 임박·진행 중·추천·후보).

2026-09-25부터 아침·저녁 점검이 같은 파이프라인이고, 이 칸들은 `track.py`가
`briefing_card()`를 불러 두 번 모두 카드 끝에 붙인다. 아래 설명은 이 파일이
저녁 전용 카드였던 시절의 것이라 역사로 남긴다. `python briefing.py`는 지금은
수동 확인용이다.
...
ENDING_DAYS = 7      # 완결 임박으로 볼 기간
BACKLOG_PICK = 3     # 하루에 집어 올릴 추천 개수
# 지금 붙잡고 있는 게 있으면 추천을 아예 내지 않는다 (2026-08-26). 원래는
# 세 개에서 한 개로 줄이기만 했는데, 하던 것을 끝내기 전에는 새 작품을
# 들이밀지 말라는 것이 사용자의 뜻이다.
#
# 다만 "얼마나 봤는지"는 게임만 알 수 있다 — 스팀이 마지막플레이일을 준다.
# 영상물에는 그런 기록이 없어서, 증거가 있는 행만 센다. 모르는 걸 넘겨짚어
# 추천을 끄면 조용해진 이유를 알 수 없게 된다. 없으면 평소대로 세 개다.
#
# 진행도가 '진행 중'인 것만으로 끄지는 않는다. 몇 달 전에 틀어놓고 잊은 행이
# 그대로 남아 있어서(폴아웃·Avowed) 그걸 기준으로 삼으면 추천이 영영 안 나온다
# — 1순위인 발굴 통로가 조용히 막힌다. 최근 BUSY_DAYS 안에 **실제로 손댄
# 기록**이 있어야 '한창 진행 중'으로 본다.
BACKLOG_PICK_BUSY = 0
...
def collect(cur, today):
    ...
    for row in unseen:
        ...
        if not av:
            continue
        # 확정이 아닌 정밀도에서 날짜는 구간의 마지막날을 넣어둔 자리표시다.
