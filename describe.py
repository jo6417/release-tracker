...
# 카드에 적을 이름. 종류 값은 태그·정렬에 쓰는 내부 이름이라 짧아야 하지만,
# 요약 줄에서는 그 짧은 이름이 무슨 일을 가리키는지 안 읽히는 것이 있다.
# `신규`가 그랬다 - 같은 카드에 `[게임패스 신규]`, `[신규 예고편]`이 나란히
# 떠서 "뭐가 신규라는 건지" 되묻게 됐다. 태그는 그대로 두고 표시만 바꾼다.
LABEL = {"신규": "작품 등록"}
...
# 작품이라 제목을 봐도 기억이 안 난다. 반대로 진행 중인 것은 지금 붙잡고
# 있는 것이라 설명이 필요 없다.
